...
//    public Map calcProductsPrice(List<OmsCartItem> cartItemList) {
//
//    }
        ...
        for (Map.Entry<Long, List<OmsCartItem>> entry : productCartMap.entrySet()) {
            ...
            if (promotionType == 1) {
                ...
                for (OmsCartItem item : itemList) {
                    ...
                    cartPromotionItem.setDiscountType(promotionType);
                    //商品原价-促销价
                ...
                if(ladder!=null){
                    for (OmsCartItem item : itemList) {
                        ...
                        cartPromotionItem.setDiscountType(promotionType);
                        cartPromotionItem.setDiscount(ladder);
                        //商品原价-折扣*商品原价
                        PmsSkuStock skuStock = getOriginalPrice(promotionProduct,item.getProductSkuId());
                        if (skuStock == null) {
                            continue;
                ...
                if(fullReduction!=null){
                    for (OmsCartItem item : itemList) {
                        ...
                        cartPromotionItem.setPromotionMessage(message);
                        cartPromotionItem.setDiscountType(promotionType);
                        cartPromotionItem.setDiscount(fullReduction);
                        //(商品原价/总价)*满减金额
                        PmsSkuStock skuStock= getOriginalPrice(promotionProduct, item.getProductSkuId());
                        if (skuStock == null) {
                            continue;
                        ...
                        BigDecimal originalPrice = skuStock.getPrice();
                        BigDecimal reduceAmount = fullReduction.getReducePrice().divide( new BigDecimal(cartPromotionItem.getQuantity()),RoundingMode.HALF_EVEN);
//                        originalPrice.divide(totalAmount,RoundingMode.HALF_EVEN).multiply(fullReduction.getReducePrice());
                        cartPromotionItem.setReduceAmount(reduceAmount);
                        cartPromotionItem.setRealStock(skuStock.getStock()-skuStock.getLockStock());
                        cartPromotionItem.setIntegration(promotionProduct.getGiftPoint());
                        cartPromotionItem.setGrowth(promotionProduct.getGiftGrowth());
